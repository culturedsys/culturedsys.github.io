...
the type, followed by the data for the value. A parser for this general format
...
with `$`; however, the `attoparsec` library will backtrack once if encounters,
...
yet, but can now do so, simply as a choice between the different parsers we have implemented:
...
which is not as much as a limitation as it might be, do as redis has separate
...
[an async chat server in Haskell](https://www.schoolofhaskell.com/user/joehillen/building-an-async-chat-server-with-conduit)